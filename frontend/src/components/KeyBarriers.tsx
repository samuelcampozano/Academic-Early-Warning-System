...
const BarrierCard = ({ title, impact, reason, suggestion }) => (
    <div className="mb-4">
        <p><strong>{title}:</strong> <span className={`font-bold ${impact === 'Alto' ? 'text-red-500' : 'text-yellow-500'}`}>[{impact}]</span></p>
        <p className="text-sm text-text-secondary my-1"><strong>¿Por qué importa?</strong> {reason}</p>
