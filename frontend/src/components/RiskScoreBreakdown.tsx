...
const RiskFactorCard = ({ icon, title, value, contribution, weight, score, variant, explanation }: RiskFactorCardProps) => {
    ...
    return (
        <div className={cn('p-4 rounded-lg border mb-4', variantClasses[variant].container)}>
            <div className="flex items-center font-bold text-lg mb-2">
                {icon}
                <span className="ml-2">{title}</span>
            </div>
            <p><strong>Valor Actual:</strong> {value}</p>
            <p><strong>Contribución al Score:</strong> <span className="text-primary font-bold">+{contribution} puntos</span> ({weight}% peso en modelo)</p>
            <div className="w-full bg-gray-200 rounded-full h-2.5 my-2">
                <div className={cn('h-2.5 rounded-full', variantClasses[variant].bar)} style={{ width: `${score}%`}}></div>
            </div>
