...
import { AlertTriangle, ShieldAlert, CheckCircle, FileText, Laptop, Users } from 'lucide-react';
...
const AlertChip = ({ alertText }: { alertText: string }) => {
  ...
  const Icon = iconMap[Object.keys(iconMap).find(key => alertText.includes(key)) || ''] || FileText;
...
const StudentRow: React.FC<StudentRowProps> = ({ student, isEven }) => {
  ...
  const { nombre, curso, nivel_riesgo, score_riesgo, alertas_principales } = student;
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="text-sm text-gray-900">{score_riesgo} <span className="text-gray-500">/ 100</span></div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        {alertas_principales.length > 3 && (
          <span className="text-xs font-medium text-gray-500">+{alertas_principales.length - 3} más</span>
        )}
      </td>
    </tr>
  );
